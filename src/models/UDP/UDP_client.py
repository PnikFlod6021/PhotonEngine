...
def broadcast_message(message):
    tx_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    tx_sock.sendto(str(message).encode("utf-8"), (UDPConstants.HOST, UDPConstants.RX_PORT))
    print(f"Sent start message: {message} to {UDPConstants.HOST}:{UDPConstants.RX_PORT}")
    tx_sock.close()
